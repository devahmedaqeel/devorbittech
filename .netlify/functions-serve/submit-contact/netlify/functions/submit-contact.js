...
try {
  require_main().config({ path: require("path").join(__dirname, "../../.env") });
} catch (_) {
}
...
function formatDateTime() {
  return (/* @__PURE__ */ new Date()).toLocaleString("en-PK", {
...
exports.handler = async (event) => {
  ...
  if (errors.length > 0) {
    return respond(422, { error: errors.join(" ") });
  }
  const { GMAIL_USER, GMAIL_APP_PASSWORD, ADMIN_EMAIL } = process.env;
  if (!GMAIL_USER || !GMAIL_APP_PASSWORD || !ADMIN_EMAIL) {
    console.error("[submit-contact] Missing env vars: GMAIL_USER, GMAIL_APP_PASSWORD, ADMIN_EMAIL");
    return respond(500, {
      error: "Email service not configured. Please contact us at official.devorbittech@gmail.com"
    });
  }
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  ...
  <!-- Header -->
  <tr>
    <td style="background:linear-gradient(135deg,#050a14 0%,#0a1628 100%);
               border-radius:12px 12px 0 0;padding:32px 36px;text-align:center;">
      <h1 style="margin:0;color:#1d8cf8;font-size:22px;font-weight:700;
                 letter-spacing:3px;text-transform:uppercase;">
        DEV <span style="color:#00d4ff;">ORBIT</span> TECH
      </h1>
      <p style="margin:6px 0 0;color:#8ea8c3;font-size:12px;letter-spacing:2px;">
  ...
  <!-- Title bar -->
  <tr>
    <td style="background:#1d8cf8;padding:14px 36px;">
  ...
  <tr>
    <td style="background:#ffffff;padding:32px 36px;border-radius:0 0 12px 12px;">
      ...
      <p style="margin:0 0 24px;color:#374151;font-size:15px;">
        You have received a new message through the <strong>Dev Orbit Tech</strong> website contact form.
      </p>
      ...
      <table width="100%" cellpadding="0" cellspacing="0"
        ...
        <tr style="background:#f9fafb;">
          <td style="padding:14px 18px;width:130px;color:#6b7280;font-size:12px;
                     font-weight:700;text-transform:uppercase;letter-spacing:1px;
                     border-bottom:1px solid #e5e7eb;">\u{1F464} Name</td>
          <td style="padding:14px 18px;color:#111827;font-size:15px;font-weight:500;
                     border-bottom:1px solid #e5e7eb;">${senderName}</td>
        </tr>

        <tr>
          <td style="padding:14px 18px;color:#6b7280;font-size:12px;font-weight:700;
          ...
          <td style="padding:14px 18px;border-bottom:1px solid #e5e7eb;">
            <a href="mailto:${senderEmail}" style="color:#1d8cf8;font-size:15px;text-decoration:none;">
              ${senderEmail}
            </a>
          </td>
        </tr>
        ...
        <tr style="background:#f9fafb;">
          ...
          <td style="padding:14px 18px;color:#111827;font-size:15px;border-bottom:1px solid #e5e7eb;">
            ${senderPhone}
          </td>
        </tr>
        ...
        <tr>
          ...
          <td style="padding:14px 18px;border-bottom:1px solid #e5e7eb;">
            <span style="background:#eff6ff;color:#1d4ed8;padding:4px 14px;
        ...
        <tr>
          <td colspan="2" style="padding:18px 18px 20px;">
            <p style="margin:0 0 10px;color:#6b7280;font-size:12px;font-weight:700;
                       text-transform:uppercase;letter-spacing:1px;">\u{1F4AC} Message</p>
            <div style="background:#f8fafc;border-left:4px solid #1d8cf8;border-radius:4px;
                         padding:16px;color:#374151;font-size:15px;line-height:1.7;
                         white-space:pre-wrap;">${senderMessage}</div>
          </td>
        </tr>
      ...
      <div style="margin-top:28px;text-align:center;">
        <a href="mailto:${senderEmail}?subject=Re:%20Your%20inquiry%20about%20${encodeURIComponent(service.trim())}"
           style="display:inline-block;background:#1d8cf8;color:#ffffff;text-decoration:none;
                  padding:13px 30px;border-radius:8px;font-size:15px;font-weight:600;letter-spacing:.5px;">
          \u21A9\uFE0F Reply to ${senderName}
        </a>
      </div>

      <p style="margin:24px 0 0;color:#9ca3af;font-size:12px;text-align:center;
                 border-top:1px solid #f3f4f6;padding-top:20px;line-height:1.6;">
        This email was sent automatically from the Dev Orbit Tech website contact form.<br/>
  ...
  const textBody = [
    ...
    "Dev Orbit Tech | Software Technology Park, University of Kotli (AJK)"
  ].join("\n");
  try {
    const info = await transporter.sendMail({
      ...
      subject: `\u{1F4EC} New Inquiry: ${service.trim()} \u2014 ${name.trim()}`,
      text: textBody,
      html: htmlBody
    });
    console.log(`[submit-contact] Email sent. MessageId: ${info.messageId} | From: ${email.trim()} | Service: ${service.trim()}`);
    return respond(200, { success: true });
  } catch (err) {
    console.error("[submit-contact] Nodemailer failed:", err.message);
    return respond(500, {
      error: "Failed to send your message. Please try again or email us at official.devorbittech@gmail.com"
    });
  }
};
//# sourceMappingURL=submit-contact.js.map
